...
```
- Mỗi lần đọc `offsetWidth` sau khi ghi style có thể buộc browser chạy synchronous layout.
...
+ <strong>HTML từ JavaScript (kiểu SPA):</strong> Việc tạo một lượng lớn DOM bằng JavaScript trên main thread thường tốn kém hơn so với HTML được parse dần từ server. Tuy nhiên SSR, Streaming SSR và Partial Hydration có thể giảm đáng kể chi phí này.
